...
export function IDXEmbed({ title = "Search Homes", minHeight = 1000 }: IDXEmbedProps) {
  ...
  if (iframeSrc) {
    return (
      <div className="w-full overflow-hidden reveal rounded-2xl border border-black/5 shadow-float bg-white">
        <iframe
          src={iframeSrc}
          title={title}
          className="w-full"
  ...
  if (widgetHtml) {
    return (
      <div
        className="w-full reveal rounded-2xl border border-black/5 shadow-float bg-white"
